...
vignette: >
  %\VignetteIndexEntry{Multiblock analysis}
...
file <- file.path(path, "behavioral.xlsx")
sheets <- excel_sheets(file)
data <- list()
for (s in sheets) {
    temp <- data.frame(read_excel(file, sheet = s))
    rownames(temp) <- temp[, 1]
    temp <- temp[, -1]
    # colnames(temp) <- c("before", "1week", "1month", "2months", "3months")
    data <- c(data, list(temp))
}
# names(data) <- c("time", "errors", "front", "all")
names(data) <- sheets
...
```{r tuning2}
ncomp <- 2
selected <- lapply(data, function(x) rep(ncol(x), 2))
```
