...
p1 <- data %>% filter(time %in% c("Before-Surgery", "1 Week")) %>% 
...
p1 <- data %>% filter(time %in% c("Before-Surgery", "1 Week")) %>% 
  ...
  theme_bw() +
  scale_x_discrete(expand=expansion(mult=0.05))+
  xlab("Visit") + ylab("Number of error") +
  theme(strip.text.x = element_text(size=12, face="bold", color ="white"),
        strip.background = element_rect( fill="gray8"))
p1
```
...
p1 <- data %>% 
  ...
  theme_bw() +
  scale_x_discrete(labels = c('1','2','3','4','5'))+
  xlab("Visit") + ylab("Racine carré de number of errors") +
  theme(strip.text.x = element_text(size=12, face="bold", color ="white"),
        strip.background = element_rect( fill="gray8"))
p1
```
...
```{r}
data <- read_excel("/home/baptiste.criniere/Documents/Oceane/Comportement R.xlsx",  skip = 2)
...
pwc %>% mutate_if(is.numeric, round, digits = 3) %>% 
