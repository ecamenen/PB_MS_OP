...
blocks <- lapply(seq(5), function(i) sapply(behavioral_tests_multiblock[-5], function(j) j[, i]))
...
res.mfa <- MFA(d, 
               ...
               name.group = c(times, "group", "donnor"), num.group.sup = c(6, 7),
               graph = FALSE)
```
...
p <- fviz_mfa_var(res.mfa1, "quanti.var", palette = "jco", col.var.sup = "violet", repel = TRUE, select.var = list(name = c("Nb. of errors","Nb. of errors ", "Nb. of errors  ", "Front limb",  "Front limb ",  "Front limb  ", "speed")))
p
```
...
fviz_mfa_ind(res.mfa1, geom = "point", habillage = "group", shape = "group",
...
fviz_mfa_ind(res.mfa, habillage = "donnor",
             palette = "jco",
             addEllipses = TRUE, ellipse.type = "confidence", 
             repel = TRUE 
             )
...
d$mouse <- rownames(d)
a <- merge(a, unique(d[,c("mouse", "group", "donnor")]), by.x = "id", by.y = "mouse")
...
p1 <- a %>% 
  ...
  xlab("Visit") + ylab("Composite Score") +
